...
//Math
int d = 1;
double pixelhight = tan(65/2) * (d/(4*sqrt(2)));
void setup(){
...
void draw(){
  ...
  for(int y=0; y<rows; y++){
    for(int x=0; x<cols; x++){
      if(depths.length >= 64) { //64
        //if((depths[x+y*cols]/10)<32){
...
void serialEvent(Serial p){
  ...
  if (!buff.startsWith("M,") && !buff.startsWith("A,")) {
    return;
  }
  println("Buff " + buff);
  
  buff = buff.substring(0, buff.length()-1).strip(); // remove the Carriage Return
  if (buff != "") {
    String[] parts = split(buff, ',');
    if (!parts[0].equalsIgnoreCase("M")) {
      return;
    }
    for (int r = 0; r < 8; r++){ 
    for (int i = 1; i < parts.length; i++) {
      depths[(i-1) + r*8] = int(parts[i].strip());
    }
    }
  }
  //println(buff);
